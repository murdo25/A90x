...
def get_welcome_response():
    ...
    session_attributes = {'difficultyLevel':0}
    card_title = "Welcome"
    speech_output = workout0.workout_start() + workout0.get_difficulty_message()
    # If the user either does not reply to the welcome message or says something
    # that is not understood, they will be prompted again with this text.
    reprompt_text = "Please choose a workout difficulty."
...
def handle_session_end_request():
    ...
    speech_output = workout.finish_workout()
...
def create_difficulty_level_attributes(difficulty_level):
    return {"difficultyLevel": difficulty_level}


def create_workout_from_difficulty(intent, session):
    """ Sets the difficulty in the session and prepares the speech to reply to the
    user.
    """
    ...
    if 'Difficulty' in intent['slots']:
        difficulty_level = intent['slots']['Difficulty']['value']
        session_attributes = create_difficulty_level_attributes(difficulty_level)
        ...
        workout = workIt(difficulty_int)
        
        workoutmessage = workout.get_next_message()
        for i in range (0, workout.get_number_of_messages()-2):
            workoutmessage = workoutmessage + "<break time=\"10s\"/>" + workout.get_next_message()
            
        speech_output = "You've chosen " + difficulty_level + workoutmessage
        
        #ENTIRE WORKOUT STRING GOES HERE WITH PAUSES BETWEEN IN SSML
                        
        reprompt_text = "What difficulty did you want?"
    else:
        speech_output = "I'm not sure what difficulty you want. " \
                        "Please try again."
        reprompt_text = "I'm not sure what difficulty you want. " \
                        "Please try again."
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, reprompt_text, should_end_session))
...
def on_intent(intent_request, session):
    ...
    if intent_name == "Workout":
        return create_workout_from_difficulty(intent, session)
